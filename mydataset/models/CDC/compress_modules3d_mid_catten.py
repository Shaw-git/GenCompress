...
from .network_components import ResnetBlock, FlexiblePrior, Downsample, Upsample, ResnetBlockAtten
...
class ResnetCompressor(Compressor):
    ...
    def build_network(self):
        ...
        for ind, (dim_in, dim_out) in enumerate(self.in_out):
            ...
            if ind == 0 :
                ResLayer = ResnetBlock(dim_in, dim_out, None, True if ind == 0 else False, d3 = d3)
            else:
                ResLayer = ResnetBlockAtten(dim_in, dim_out, None, True if ind == 0 else False, d3 = d3)
        ...
        for ind, (dim_in, dim_out) in enumerate(self.reversed_in_out):
            ...
            if is_last :
                ResLayer = ResnetBlock(dim_in, dim_out if not is_last else dim_in, d3 = d3)
            else:
                ResLayer = ResnetBlockAtten(dim_in, dim_out if not is_last else dim_in, d3 = d3)
