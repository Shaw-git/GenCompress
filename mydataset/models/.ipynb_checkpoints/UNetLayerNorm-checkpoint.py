...
class UNetWithChannelAttention(nn.Module):
    def __init__(self, in_channels, out_channels, catten=False):
        ...
        # Encoder
        self.encoder1 = self._block(in_channels, 64, size= 240)      # In: [b, 1, 240, 240], Out: [b, 64, 240, 240]
        self.encoder2 = self._block(64, 96, stride=2, size= 120)    # In: [b, 64, 240, 240], Out: [b, 96, 120, 120]
        self.encoder3 = self._block(96, 128, stride=2, size= 60)   # In: [b, 96, 120, 120], Out: [b, 128, 60, 60]
        self.encoder4 = self._block(128, 256, stride=2, size= 30)  # In: [b, 128, 60, 60], Out: [b, 256, 30, 30]
        
        # Middle
        self.middle = self._block(256, 256, stride=2, size=15)   # In: [b, 256, 30, 30], Out: [b, 256, 15, 15]
        ...
        if catten:
            ...
            self.ca4, self.ca3, self.ca2, self.ca1 = [ReturnSelf() for _ in range(4)]
        
        # Decoder
        self.upconv4 = self._upconv(256, 256)  # In: [b, 256, 15, 15], Out: [b, 256, 30, 30]
        self.decoder4 = self._block(512, 256, size= 30) # After concatenation: [b, 512, 30, 30], Out: [b, 256, 30, 30]
        
        self.upconv3 = self._upconv(256, 128)  # In: [b, 256, 30, 30], Out: [b, 128, 60, 60]
        self.decoder3 = self._block(256, 128, size= 60) # After concatenation: [b, 256, 60, 60], Out: [b, 128, 60, 60]
        
        self.upconv2 = self._upconv(128, 96)   # In: [b, 128, 60, 60], Out: [b, 96, 120, 120]
        self.decoder2 = self._block(192, 96, size= 120)  # After concatenation: [b, 192, 120, 120], Out: [b, 96, 120, 120]
        
        self.upconv1 = self._upconv(96, 64)    # In: [b, 96, 120, 120], Out: [b, 64, 240, 240]
        self.decoder1 = self._block(128, 64, size= 240)  # After concatenation: [b, 128, 240, 240], Out: [b, 64, 240, 240]
        
        self.out_conv = nn.Conv2d(64, out_channels, kernel_size=1) # In: [b, 64, 240, 240], Out: [b, out_channels, 240, 240]

    def _block(self, in_channels, out_channels, stride=1, size = 1):
        return ResidualBlock(in_channels, out_channels, stride=stride, size = size)
...
class ResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, size = -1):
        super(ResidualBlock, self).__init__()
        self.conv1 = nn.Conv2d(in_channels, out_channels, kernel_size=3, padding=1, stride=stride)
        self.conv2 = nn.Conv2d(out_channels, out_channels, kernel_size=3, padding=1)
        self.layer_norm = nn.LayerNorm([out_channels, size, size])  # 240 is for input size 240x240
