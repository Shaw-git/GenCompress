...
class S3DLatent(BaseDataset):
    def __init__(self, args):
        super().__init__(args)
        self.dataset_name = "S3D Latent"
        
        var = args["var"] if "var" in args else range(10, 58)
        
        self.return_norm = args["return_norm"] if "return_norm" in args else False
        
        self.data_input = self.load_s3d_dataset(self.data_path, var)
    ...
    def load_s3d_dataset(self, data_path, var):
    ...
    def __getitem__(self, idx):
        ...
        if self.inst_norm:
            ...
            if self.return_norm:
                a2,b2 = data[1], data[2]
                a1,b1 = self.var_mean.reshape(-1)[idx0], self.var_scale.reshape(-1)[idx0]
                
                offset, scale = a1+b1*a2, b1*b2
                return data[0], offset, scale, 1
